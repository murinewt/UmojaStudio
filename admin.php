...
<?php
            if(isset($_POST['adminPass'])){
                ...
                $specialPassword = $_POST['specialPassword'];
                //$_SESSION['time']=$datein;
                $salt="R6pJC3gT5?#";
                $encPass=md5($salt.$password);
                $passsql = "SELECT * FROM umojaadmin WHERE username='$username' and password='$encPass'";
                $passresult = mysqli_query($db, $passsql);
                $getThat = mysqli_fetch_array($passresult);
                
                $dataName = $getThat['username'];
                $dataPass = $getThat['password'];
                            ...
                            if($dataPass==$encPass){
                                if($specialPassword=="qwertyuiop"){
                                    ...
                                    if($userCount==0){
                                        ...
                                        $_SESSION['time']=$datein;
                                        $anothersql = "INSERT INTO login (username,login_time,logout_time) VALUES ('$username','$datein','$datein')";
                                ...
                                echo"<script type='text/javascript'>alert('input correct password!');</script>";
                            }
                        }
                        else{
                            echo"<script type='text/javascript'>alert('input correct username!');</script>";
                        }
                    }
                    else{
...
<body>
    ...
    <div class="login-clean">
        <form method="post">
            <h1 class="text-center" style="color:rgb(30,131,232);">Admin Login</h1>
            ...
            <div class="form-group"><input class="form-control" type="password" name="specialPassword" placeholder="Special-Password"></div>
            <div class="form-group"><button class="btn btn-primary btn-block" type="submit" name="adminPass" style="background-color:rgb(239,120,11);">Log In</button></div><a href="#" class="forgot">Forgot your username or password?</a></form>
    </div>
                ...
                <div class="col-md-4">
                    ...
                    <p
                        ...
                        <div class="d-inline-block intro">
                            ...
                            <p class="text-center" style="font-size:20px;">To be updated on our locations and what we are doing enter your username for automatic updates</p>
                        </div>
                        <form class="form-inline d-flex justify-content-center" method="post">
                            <div class="form-group"><input class="form-control" type="username" name="username" placeholder="Your username"></div>
